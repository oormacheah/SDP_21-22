...
static void *sum_thread (void *arg) {
  ...
  while(1) {
    sem_wait(barrier1); // wait for all threads to reach this part to start
    if (step > n)
      break;
    gap = 1 << (step-1); // first, distance 1, then distance 2, then 4, 8 and so on...
    if (i >=  gap ) // start being able to sum with the (i-gap)-th element
      sum = x[i] + x[i-gap];

    sem_wait(me); // mutex basically, just one allowed at a time 
    printf ("    step %ld, i %ld, counter %ld, active = %ld\n",
      step, i, counter, active_threads);
    if (++counter == active_threads){ // basically if you're the last thread start unlocking every other thread
      for (j=0;j<active_threads;j++)
        sem_post(barrier2);
    }
    sem_post(me);
  
    sem_wait(barrier2); // wait until the last thread
    x[i] = sum; // remember each thread carries it's own index
    printf ("    step %ld, i %ld, active %ld, x[%ld]=sum = %ld\n",
      step, i, active_threads, i, sum);

    sem_wait(me); // mutex again
    if (--counter == 0) { // last thread will enter
      printf ("    last is %ld\n", i);
      if (i == step) { // first thread had 1
        printf("    step %ld , thread %ld exiting as last, active %ld\n",
	  step, i, active_threads-1);   
        step++;
        --active_threads;
        for (j=0; j<active_threads; j++) {
          sem_post(barrier1); // unlock the following iterations
	      }
        sem_post(me);
        pthread_exit ((void *) sum);
       } else { // if it is the last one, but not the threa i == step
        step++;
        for (j=0; j<active_threads; j++)
          sem_post (barrier1);
       }
    } 
    else { // all threads enter here before
      if (i == step) {
        --active_threads;
        sem_post(me);
